...
public class UiDevice implements Searchable {
    ...
    /**
     * 根据在屏幕中的绝对位置来做点击，click()方法是相对的window窗口的位置
     *
     * @param x coordinate
     * @param y coordinate
     * @return true if the click succeeded else false
     */
    public boolean clickInScreen(int x, int y) {
        Tracer.trace(x, y);
        if (x >= getDisplayWidth() || y >= getDisplayRealHeight()) {
            return (false);
        }
        return getInteractionController().clickNoSync(x, y);
    }
    ...
    public static UiAutomation getUiAutomation(final Instrumentation instrumentation) {
